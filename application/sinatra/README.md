# Sinatra application in Docker
This sinatra example checks if the environment variables DEMO_USERNAME and DEMO_PASSWORD are set. If they are, the application responds (at localhost:8080) to the requests with a status code 200 and a welcome message. If not, the application responds with status code 500.
...
```
docker build . -t sinatra-secrethub-demo
```
